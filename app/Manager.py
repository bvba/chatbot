...
class Manager():
    ...
    def process(self, s, req=None):
        if s == 'keyboard':
            ...
            # 따라서 현재 날짜(tm.mainTIme)의
            # keybod(default keybod == include ' ')를 반환한다
            ...
            if content in src.mealTime:
                resp['message'] = {
                    'text': menu.getMeal(content, dm.getUserTime(user_key))}
            # '이전 날로 날짜 변경'
            ...
            # '소개'
                ...
                resp['message'] = {
                    'text': src.intro, 'message_button': {
                        'label': '문의하기', 'url': src.openchatLink
                        }
                }
            else:
                resp['message'] = {
